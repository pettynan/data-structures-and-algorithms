...
const hasChildrenValues = (arr, character) => {
  ...
  for (let i = 0 ; i < arr.length ; i++){
    ...
    if (arr[i].name === character) {
      if ((Object.values(arr[i])[2].length)) {
        return true;
      } else {
        return false;
      }
...
const totalCharacters = (arr) => {
  ...
  for (let i = 0 ; i < arr.length ; i++) {
    ...
    if (arr[i].spouse) {
      total += arr[i].children.length + 2;
    } else {
      total += arr[i].children.length + 1;
    }
  }
...
const houseSize = (arr) => {
  ...
  for (let i = 0 ; i < arr.length ; i++) {
    let members = arr[i].children.length + 1;
    if (arr[i].spouse) {
      members += 1;
    }

    sizes.push({house: arr[i].house, members: members})
  }
  return sizes;
};
