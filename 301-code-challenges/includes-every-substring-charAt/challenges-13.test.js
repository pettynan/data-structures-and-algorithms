...
const standardizePhoneNumbers = (arr) => {
  return arr.map(str => `${str.substr(1, 3)}${str.substr(6, 3)}${str.substr(10, 4)}`);
};
...
const findAnything = (arr, target) => {
  return arr.filter(x => x.includes(target));
};
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.forEach(day => {
    ans[daysOfWeek.indexOf(day)] = arr.filter(x => x.includes(day));
  });
