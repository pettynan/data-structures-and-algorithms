...
const extractVowels = (str) => {
  let regEx = /[aeiou]/g;
  let vowelsArray = str.match(regEx);

  // Borrowed sort code from challenges-03 Challenge #4
  vowelsArray.sort( (a, b) => {
  ...
  for (let i = 0 ; i < vowelsArray.length ; i++) {
    vowelsString = `${vowelsString}${vowelsArray[i]}`
  }

  return [str.replace(regEx, ''), vowelsString];
};
