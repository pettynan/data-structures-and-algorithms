...
const totalSum = (input) => {
  return input.map( x => x.reduce((a, b) => a + b)).reduce((a, b) => a + b);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(x => x.filter(y => y % 5 === 0 && y === parseInt(y)).map(z => Math.pow(2, z)));
};
...
let findMaleAndFemale = (data) => data.filter(x => x.gender === 'male' || x.gender === 'female').map(y => y.name).reduce( (ansSoFar, current) =>`${ansSoFar} and ${current}`);
